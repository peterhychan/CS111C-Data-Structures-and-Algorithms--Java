...
public boolean equals(ListInterface another){
	ListInterface<T> equals = new AList<T>();
	AList<T> otherBag = (AList<T>) another;
	boolean output = false; //default 
	int length = getLength();
	int other_length = another.getLength();
	if (length ==other_length){
		for (int i =1;  length != i; i++){
			T one = getEntry(i);
			T two = otherBag.getEntry(i);
			output = (one == two);
		}
	}
	return output;
